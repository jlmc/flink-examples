...
public class ShufflePartitionerExampleIT {
    ...
    private static final String INPUT_TOPIC = "heavy-logs";
    private static final String OUTPUT_TOPIC = "processed-results";
    ...
    @Test
    void shouldDistributeLoadAcrossMultipleSubtasksUsingShuffle() throws Exception {
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(producerProps)) {
            for (int i = 0; i < messageCount; i++) {
                producer.send(new ProducerRecord<>(INPUT_TOPIC, "log-message-" + i));
            }
            producer.flush();
        }
        ...
        ShufflePartitionerExample.execute(new String[0]);
        ...
        try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(consumerProps)) {
            ...
            await().atMost(Duration.ofSeconds(60)).untilAsserted(() -> {
                consumer.poll(Duration.ofMillis(100)).forEach(record -> {
                    ...
                    messages.add(value);
                    // Extract subtaskIndex from JSON: {"original": "...", "processed": "...", "subtaskIndex": N}
                ...
                // Verify that multiple subtasks were involved (since we set parallelism to 4 and used shuffle)
                assertTrue(subtaskIndices.size() > 1, "Expected data to be processed by multiple subtasks. Subtasks found: " + subtaskIndices);
