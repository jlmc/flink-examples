# Flink Kafka Sink JSON Example

This module provides an example of using the Flink Kafka Sink connector to write JSON messages to a Kafka topic using Flink's provided `JsonSerializationSchema` (from the `flink-json` module).
...
In the `KafkaSinkJsonExample.java`, checkpointing is enabled as follows:
...
- `kafka-init` (to create the `json-topic` topic)
