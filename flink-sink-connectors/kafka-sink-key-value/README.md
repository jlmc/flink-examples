# Flink Kafka Sink Key-Value Example

This module provides an example of using the Flink Kafka Sink connector to write Key-Value messages to a Kafka topic.
...
In the `KafkaSinkKeyValueExample.java`, checkpointing is enabled as follows:
...
- `kafka-init` (to create the `key-value-topic` topic)
...
```bash
docker exec -it kafka kafka-console-consumer --bootstrap-server localhost:9092 --topic key-value-topic --from-beginning --max-messages 10 --property print.key=true --property key.separator=":"
```
