...
# Find the shaded JAR
JAR_FILE=$(find "$DIR/target" -name "s3-sink-connector-*-shaded.jar" | head -n 1)
...
#   RUN_RESPONSE=$(curl -s -X POST "http://localhost:8081/jars/$JAR_ID/run?program-args=--s3.endpoint+${S3_ENDPOINT}+--s3.access-key+${S3_ACCESS_KEY}+--s3.secret-key+${S3_SECRET_KEY}")
