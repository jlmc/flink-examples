...
class ProcessWindowFunctionExampleTest {
    ...
    @Test
    void shouldProcessAndCountElementsPerWindow() throws Exception {
        ...
        List<SensorReading> input = List.of(
                ...
                new SensorReading("s1", now.plus(Duration.ofMinutes(1)), 30.0),
                new SensorReading("s1", now.plus(Duration.ofMinutes(11)), 100.0)
        );
        ...
        ProcessWindowFunctionExample.definePipeline(stream)
                .addSink(new CollectSink<>());

        env.execute("ProcessWindowFunctionExampleTest");

        List<String> results = CollectSink.values();
        assertThat(results).hasSize(2);
        assertThat(results).anyMatch(s -> s.contains("Count: 2"));
        assertThat(results).anyMatch(s -> s.contains("Count: 1"));
    }
}
